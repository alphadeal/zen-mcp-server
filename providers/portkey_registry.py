"""Portkey model registry for managing model configurations and aliases."""
...
class PortkeyModelRegistry:
    """Registry for managing Portkey model configurations and aliases."""
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        if config_path:
            ...
            env_path = os.getenv("PORTKEY_MODELS_CONFIG_PATH")
            if env_path:
                ...
                try:
                    resource_traversable = importlib.resources.files("conf").joinpath("portkey_models.json")
                ...
                if not self.use_resources:
                    ...
                    potential_paths = [
                        Path(__file__).parent.parent / "conf" / "portkey_models.json",
                        Path.cwd() / "conf" / "portkey_models.json",
                    ]
    ...
    def reload(self) -> None:
        ...
        try:
            ...
            logging.debug(
                f"Loaded {len(self.model_map)} Portkey models with {len(self.alias_map)} aliases{caller_info}"
            )
        except ValueError as e:
            # Re-raise ValueError only for duplicate aliases (critical config errors)
            logging.error(f"Failed to load Portkey model configuration: {e}")
    ...
    def _read_config(self) -> list[ModelCapabilities]:
        ...
        try:
            if self.use_resources:
                ...
                try:
                    resource_path = importlib.resources.files("conf").joinpath("portkey_models.json")
                    ...
                    logging.debug("Loaded Portkey config from package resources")
                except Exception as e:
                    logging.warning(f"Failed to load Portkey config from resources: {e}")
                    return []
            else:
                # Use file path loading
                if not self.config_path.exists():
                    logging.warning(f"Portkey model config not found at {self.config_path}")
                    return []

                # Use centralized JSON reading utility
                data = read_json_file(str(self.config_path))
                logging.debug(f"Loaded Portkey config from file: {self.config_path}")
            ...
            for model_data in data.get("models", []):
                ...
                model_data.setdefault("provider", ProviderType.PORTKEY)
                model_data.setdefault("friendly_name", f"Portkey ({model_data.get('model_name', 'Unknown')})")
            ...
            location = "resources" if self.use_resources else str(self.config_path)
            raise ValueError(f"Error reading Portkey config from {location}: {e}")
